...
It is highly recommended that you follow Rob's tutorials if you are new to FEA in NASTRAN, as they provide step-by-step guides for simulation cases identical to PAST.
...
- Common contacts are Separation, Sliding and Bonded (all of which are what it says on the tin) - a good approach is to think about 
- Under-constrained contacts tend to lead to weird behaviours. Be sure that you've constrained all motions in the way you'd expect - Think about it like if you don't tell NASTRAN that a cup should have a separation contact with the top of a table, your simulation will end with coffee all over the floor.

Connectors:
- Usually the geometry of fasteners like bolts, screws and nuts is to complex to be included in an analysis. Exclude them from your analysis - don't add a screw 3D model you got online. In order to simulate them, we can add a connector in NASTRAN.
...
- Once you select "Bolt" the dialogue box will ask you to fill in the parameters for your bolt. Rob's tutorials (see the end of this document) provide a good guide for selecting bolt parameters, but a wide number of calculators and tools exist to help you. In general, you mostly care about the bolt parameter, and setting a preload that's sane and sufficient for your needs, but doesn't lead to massive stresses on your analysis
...
- You can create multiple seperate load cases for one analysis. For example, you might be interested in seeing how a structure responds to a heavy load in one area and a light load in another. Running the analysis will produce results for each load case.
...
- [Rob's Tutorials. **HIGHLY RECCOMENDED!**](https://curtin.sharepoint.com/:f:/r/sites/CurtinUndergraduateCubeSatTeam/Shared%20Documents/General/Technical%20Departments/Technical%20Resources/Mechanical%20Design%20%26%20Analysis?csf=1&web=1&e=x2TH6c)
